...
$csv->setActiveSheetIndex(0)->setCellValue('F1', "Jumlah Req"); // Set kolom F1 dengan tulisan "ALAMAT"
...
$sql = mysqli_query($conn, "select * from req inner join reqdata on req.nota=reqdata.nota where req.cabang like '%$cabang%' and req.unit like '%$unit%' and (req.tglreq BETWEEN '$awal' and '$akhir') group by reqdata.kode");
...
while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
	...
	$csv->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $akhir);
	...
	$csv->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $row['data']);
...
$kode = $data['kode'];
	...
	$kode = $data['kode'];
            $sqlx2="SELECT MAX(reqdata.stok) AS data3 FROM req inner join reqdata on req.nota=reqdata.nota where reqdata.kode='$kode' and req.cabang like '%$cabang%' and req.unit like '%$unit%' and (req.tglreq BETWEEN '$awal' and '$akhir') ";
    $hasilx2=mysqli_query($conn,$sqlx2);
    $row=mysqli_fetch_assoc($hasilx2);
    
	
	$csv->setActiveSheetIndex(0)->setCellValue('I'.$numrow, $row['data3']);
